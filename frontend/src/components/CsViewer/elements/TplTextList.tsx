...
import {
    CharTemplateItem, centPublish, currentCs, isCsOwner, netTopic,
    setCurrentCs, topicCsInfo, updateCsStorage
} from "~/common";
...
export const TplTextList: Component<Props> = ({ item, checkable }) => {
    ...
    return <Flex direction="column">
        <Flex style={{ "justify-content": "space-between" }}>
            <Text colorSchema="secondary" fontSize="smaller">{item.name}</Text>
            <div class={csTplIconStyle}
                title="Add item" onClick={addItem}>
                <FaSolidPlus fill="currentColor" />
            </div>
        </Flex>
        <For each={values()}>{(it, idx) =>
        (
            <Show when={it}>
                <Flex style={{ "justify-content": "space-between" }}>
                        ...
                        <Flex style={{ "align-items": "center" }} gap="medium">
                            <Show when={checkable}>
                                <TplCheckBlock
                                    ...
                                    circle={false}
                                    color={item.color}
                                    onClick={isCsOwner(currentCs()) ? () => toggleCheck(idx()) : undefined}
                                />
                            </Show>
                            <Text>{it.text}</Text>
                        </Flex>
                    ...
                    <Show when={isCsOwner(currentCs()) && editedItem() !== idx()}>
                        <Flex>
                            <div class={csTplIconStyle}
                                title="Edit item" onClick={() => toggleEdit(idx(), true)}>
                                <FaSolidPen fill="currentColor" />
                            </div>
                            <div class={csTplIconStyle}
                                title="Delete item" onClick={() => delItem(idx())}>
