...
import { addCheckControl, addRollControl, removeStdControls } from "./control";
...
export const createFromTemplate = (
  ...
  fabric.Image.fromURL(page.img, (img) => {
    ...
    if (img.width && img.width > mw) {
      mw = img.width;
      if (canvas.width && canvas.width < mw) canvas.setWidth(mw);
    }
    if (img.height && img.height > mh) {
      mh = img.height;
      if (canvas.height && canvas.height < mh) canvas.setHeight(mh);
    }
  });
