...
// QueryRolls chains the current query on the "rolls" edge.
func (uq *UserQuery) QueryRolls() *RollQuery {
	query := (&RollClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryCharsheets chains the current query on the "charsheets" edge.
func (uq *UserQuery) QueryCharsheets() *CharsheetQuery {
	query := (&CharsheetClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithRolls tells the query-builder to eager-load the nodes that are connected to
// the "rolls" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithRolls(opts ...func(*RollQuery)) *UserQuery {
	query := (&RollClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithCharsheets tells the query-builder to eager-load the nodes that are connected to
// the "charsheets" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithCharsheets(opts ...func(*CharsheetQuery)) *UserQuery {
	query := (&CharsheetClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withRolls; query != nil {
		if err := uq.loadRolls(ctx, query, nodes,
			func(n *User) { n.Edges.Rolls = []*Roll{} },
			func(n *User, e *Roll) { n.Edges.Rolls = append(n.Edges.Rolls, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withCharsheets; query != nil {
...
func (uq *UserQuery) loadRolls(ctx context.Context, query *RollQuery, nodes []*User, init func(*User), assign func(*User, *Roll)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Roll(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.RollsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_rolls
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_rolls" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_rolls" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UserQuery) loadCharsheets(ctx context.Context, query *CharsheetQuery, nodes []*User, init func(*User), assign func(*User, *Charsheet)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Charsheet(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.CharsheetsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_charsheets
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_charsheets" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
