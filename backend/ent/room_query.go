...
func (rq *RoomQuery) QueryRolls() *RollQuery {
	query := (&RollClient{config: rq.config}).Query()
...
func (rq *RoomQuery) WithRolls(opts ...func(*RollQuery)) *RoomQuery {
	query := (&RollClient{config: rq.config}).Query()
...
func (rq *RoomQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Room, error) {
	...
	if query := rq.withRolls; query != nil {
		if err := rq.loadRolls(ctx, query, nodes,
			func(n *Room) { n.Edges.Rolls = []*Roll{} },
			func(n *Room, e *Roll) { n.Edges.Rolls = append(n.Edges.Rolls, e) }); err != nil {
			return nil, err
		}
	}
...
func (rq *RoomQuery) loadRolls(ctx context.Context, query *RollQuery, nodes []*Room, init func(*Room), assign func(*Room, *Roll)) error {
	...
	query.Where(predicate.Roll(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(room.RollsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.room_rolls
		if fk == nil {
			return fmt.Errorf(`foreign-key "room_rolls" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "room_rolls" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (rq *RoomQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Room, init func(*Room), assign func(*Room, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_rooms == nil {
			continue
		}
		fk := *nodes[i].user_rooms
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_rooms" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
